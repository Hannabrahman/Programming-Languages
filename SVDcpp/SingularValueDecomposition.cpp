...
/* Main function that do the decomposition of Matrix a */
int SVD::dsvd(Matrix<float> &a, int m, int n, float *w, Matrix<float> &v)
{
...
void SVD::ReadFile(const char* in_file){
...
void SVD::WriteFile(const char* out_file){
  // write V and S to output file
  std::string v_out = out_file;
  v_out += "/matrix_cppsvd_V.output";
  ofstream f1(v_out.c_str());

  std::string s_out = out_file;
  s_out += "/matrix_cppsvd_S.output";
  ofstream f2(s_out.c_str());

  std::string u_out = out_file;
  u_out += "/matrix_cppsvd_U.output";
  ofstream f3(u_out.c_str());
